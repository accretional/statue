...
<script lang="ts">
   ...
   */
  function scaleBandInvert(scale: ScaleBand<any>) {
   ...
   */
  function scaleInvert(scale: AnyScale<any, any>, value: number) {
  ...
  // Inlined from $lib/utils/quadtree.js
  type Rect = { x: number; y: number; width: number; height: number };

  /**
   * Transverse quadtree and generate rect dimensions
   */
  function quadtreeRects<T>(quadtree: Quadtree<T>, showLeaves = true) {
    const rects: Rect[] = [];
