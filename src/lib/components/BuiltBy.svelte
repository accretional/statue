...
<div class="built-by-container flex flex-wrap justify-center items-center gap-2">
	...
	<a
		...
		class="built-link accretional-link inline-flex items-center gap-1"
	>
		<img
			src={builtByIcon}
			alt={builtByLinkText}
			class="accretion-icon"
			width="24"
			height="24"
		/>{builtByLinkText}
	</a>
	...
	<span class="discord-link inline-flex items-center gap-1">
		<svg
			...
			></path></svg
		>
		<a href={communityLinkUrl} target="_blank" class="discord-link-text underline"
			>{communityLinkText}</a
		>
	</span>
</div>
