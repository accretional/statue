...
# Test local statue-ssg package with the developer-portfolio template
# Usage: ./scripts/test-local-developer-portfolio.sh

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
TEST_DIR="$PROJECT_ROOT/build/test-developer-portfolio"
